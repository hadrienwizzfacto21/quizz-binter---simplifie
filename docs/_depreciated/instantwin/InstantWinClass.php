...
// // WIP NOTES : draw, cueadd, cuewin ok but pb keeping track of the prize = session bug ?
class InstantWinClass
{
    ...
    public function __construct($cookiesDuration = 86400)
    {
        // START SESSION
        ...
        $this->iwFile = json_decode(file_get_contents(__DIR__ . "/iwPrizesList.json", true), true);
        $this->ucFile = json_decode(file_get_contents(__DIR__ . "/iwCodesList.json", true), true);
    ...
    {
        if (!function_exists('StocksCalcul')) {
            function StocksCalcul($count)
            {
                if (substr($count, 0, 6) == date("ymd")) return $count;
            }
        }

        $prizeWonCount = array_filter($this->iwFile[$pTarget]["Won"], "StocksCalcul");
        $prizeCueCount = array_filter($this->iwFile[$pTarget]["Cue"], "StocksCalcul");
